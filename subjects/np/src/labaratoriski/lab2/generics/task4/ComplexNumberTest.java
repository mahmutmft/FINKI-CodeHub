package labaratoriski.lab2.generics.task4;
...
class ComplexNumber<T extends Number,U extends Number> implements Comparable<ComplexNumber<?,?>>{
    ...
    public double modul(){
        return sqrt(Math.pow(real.doubleValue(),2) + Math.pow(imaginary.doubleValue(),2));
    }
    ...
    public String toString(){
        String operator = "+";
        if(imaginary.doubleValue() < 0) operator = "";
        DecimalFormat df = new DecimalFormat("0.00");
        return df.format(real) + operator + df.format(imaginary) + "i";
    }
